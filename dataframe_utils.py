...
def clean_header(header, null_default=""):
    names = [name if pd.notnull(name) else null_default for name in header]
    name_counts = Counter(names)
    
    return [
        str(name) if name_counts[name] <= 1 else f"{name}({i})" for (i,name) in enumerate(names)
    ]
        
def get_dataframe_with_header(dataframe, header):
    col_names = clean_header(header)
    dataframe.columns = col_names
    return dataframe
        
def drop_null_cols(dataframe):
    cols_to_keep = [name for name in dataframe.columns if dataframe[name].notnull().any()]
    return dataframe[cols_to_keep]

def drop_null_rows(dataframe):
    notnull_rowmask = dataframe.apply(lambda row: row.notnull().any(), axis=1)
    return dataframe.loc[notnull_rowmask]
...
def load_excel(file, sheet_name=None, header_position=None):
    ...
    cleaned_dataframe = strip_spaces(raw_dataframe)
    
    if header_position is None:
        start_index, header = find_header(cleaned_dataframe)
    else:
        start_index = header_position + 1
        header = cleaned_dataframe.loc[header_position]
        
        
    dataframe_with_header = get_dataframe_with_header(cleaned_dataframe.loc[start_index:], header)
    return drop_nulls(dataframe_with_header).reset_index(drop=True)


def remap_columns(dataframe, mapping, drop_unmapped=True):
    remapped_dataframe = dataframe.rename(columns=mapping)
...
def save_as_excel(dataframe, filename, **kwargs):
    append_df_to_excel(filename, dataframe.T, **kwargs)
